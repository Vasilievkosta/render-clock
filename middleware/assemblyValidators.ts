...
const validateTime = body('time').custom((value, { req }) => {
...
const validateEmail = body('email').isEmail().isLength({ max: 50 }).withMessage('Email length should be max 50 symbols')
...
const validateDuration = body('duration').custom((value) => {
